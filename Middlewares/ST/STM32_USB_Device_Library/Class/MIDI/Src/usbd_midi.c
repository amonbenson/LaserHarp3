...
__ALIGN_BEGIN static uint8_t USBD_MIDI_CfgHSDesc[]  __ALIGN_END =
{
};

/* USB HID device Other Speed Configuration Descriptor */
__ALIGN_BEGIN static uint8_t USBD_MIDI_OtherSpeedCfgDesc[]  __ALIGN_END =
{
};
...
__ALIGN_BEGIN static uint8_t USBD_MIDI_Desc[USB_MIDI_DESC_SIZ]  __ALIGN_END  =
{
  ...
  MIDI_DESCRIPTOR_TYPE, /*bDescriptorType: HID*/
  0x11,         /*bcdHID: HID Class Spec release number*/
  0x01,
  0x00,         /*bCountryCode: Hardware target country*/
  0x01,         /*bNumDescriptors: Number of HID class descriptors to follow*/
  0x22,         /*bDescriptorType*/
  USB_MIDI_MOUSE_REPORT_DESC_SIZE,/*wItemLength: Total length of Report descriptor*/
  0x00,
};
...
__ALIGN_BEGIN static uint8_t USBD_MIDI_MOUSE_ReportDesc[]  __ALIGN_END =
{
  0x05,   0x01,
  0x09,   0x02,
  0xA1,   0x01,
...
{
  /* Open EP IN */
  USBD_LL_OpenEP(pdev, MIDI_EPIN_ADDR, USBD_EP_TYPE_INTR, MIDI_EPIN_SIZE);
  ...
  ((USBD_MIDI_HandleTypeDef *)pdev->pClassData)->state = USBD_MIDI_IDLE;
    ...
    case USB_REQ_TYPE_CLASS :
      switch (req->bRequest)
      {
        ...
        case MIDI_REQ_GET_PROTOCOL:
          USBD_CtlSendData(pdev, (uint8_t *)(void *)&hmidi->Protocol, 1U);
          break;
        ...
        case MIDI_REQ_GET_IDLE:
          USBD_CtlSendData(pdev, (uint8_t *)(void *)&hmidi->IdleState, 1U);
          break;
    ...
    case USB_REQ_TYPE_STANDARD:
      switch (req->bRequest)
      {
        ...
        case USB_REQ_GET_DESCRIPTOR:
          ...
          {
            len = MIN(USB_MIDI_MOUSE_REPORT_DESC_SIZE, req->wLength);
            pbuf = USBD_MIDI_MOUSE_ReportDesc;
          }
          else if (req->wValue >> 8 == MIDI_DESCRIPTOR_TYPE)
          {
            pbuf = USBD_MIDI_Desc;
        ...
        case USB_REQ_GET_INTERFACE :
          if (pdev->dev_state == USBD_STATE_CONFIGURED)
          {
            USBD_CtlSendData(pdev, (uint8_t *)(void *)&hmidi->AltSetting, 1U);
          }
          else
          {
            USBD_CtlError(pdev, req);
                            ...
                            uint16_t len)
{
  ...
  if (pdev->dev_state == USBD_STATE_CONFIGURED)
  {
    if (hmidi->state == USBD_MIDI_IDLE)
    {
      hmidi->state = USBD_MIDI_BUSY;
...
uint32_t USBD_MIDI_GetPollingInterval(USBD_HandleTypeDef *pdev)
{
  ...
  if (pdev->dev_speed == USBD_SPEED_HIGH)
  {
    ...
    polling_interval = (((1U << (MIDI_HS_BINTERVAL - 1U))) / 8U);
  }
  else   /* LOW and FULL-speed endpoints */
  {
    /* Sets the data transfer polling interval for low and full
    speed transfers */
    polling_interval =  MIDI_FS_BINTERVAL;
  }
